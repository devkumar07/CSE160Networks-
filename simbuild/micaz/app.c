...
#line 243
                    ...
                    if (Node__sockets[sim_node()][index].state == LISTEN || Node__sockets[sim_node()][index].state == ESTABLISHED) {
                        ...
                        __nesc_hton_uint16(Node__sockets[sim_node()][index].dest.addr.nxdata, temp->srcNode);
                        __nesc_hton_uint8(Node__sockets[sim_node()][index].dest.port.nxdata, temp->srcPort);
...
#line 271
                      ...
                      Node__sockets[sim_node()][index].effectiveWindow = temp->effectiveWindow;
...
#line 299
                          ...
                          sim_log_debug(125U, TRANSPORT_CHANNEL, "username: %s\n", temp->username);
                          sim_log_debug(126U, TRANSPORT_CHANNEL, "message: %s\n", temp->message);
                          if (Node__sockets[sim_node()][temp->destPort].state == SYN_RCVD) {
                              sim_log_debug(127U, TRANSPORT_CHANNEL, "Changing receiver state to established\n");
                              ...
                              sim_log_debug(128U, TRANSPORT_CHANNEL, "ACK init Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, temp->destPort, __nesc_ntoh_uint16(myMsg->src.nxdata), temp->srcPort, temp->seqNum);
...
#line 328
                                sim_log_debug(129U, TRANSPORT_CHANNEL, "in here tcp received\n");
                                sim_log_debug(130U, TRANSPORT_CHANNEL, "message in tcp received: %s\n", temp->message);
                                for (i = 0; i < Node__ClientsDB__size(); i++) {
                                    ConnectedClients t = Node__ClientsDB__get(i);

#line 332
                                    if (t.destNode != __nesc_ntoh_uint16(myMsg->src.nxdata)) {
