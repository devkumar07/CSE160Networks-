...
#line 742
typedef int (*__compar_fn_t)(const void *arg_0x7fce98f1a020, const void *arg_0x7fce98f1a320);
#line 780
__extension__ 
#line 797
__extension__ 
...
#line 250
extern double erfc(double arg_0x7fce98e925e0) __attribute((__leaf__)) __attribute((__nothrow__)) ;
#line 326
__extension__ 
...
unsigned int (*hashfunction)(void *arg_0x7fce98c353a0), 
int (*key_eq_fn)(void *arg_0x7fce98c35b70, void *arg_0x7fce98c35e30));
#line 103
#line 102
int 
...
static inline void RandomInitialise(int arg_0x7fce98a22c00, int arg_0x7fce98a22e80);
static double RandomUniform(void );
...
#line 355
                                  sim_log_debug(134U, TRANSPORT_CHANNEL, "from %s to %s\n", usr, receiver);
                                  sim_log_debug(135U, TRANSPORT_CHANNEL, "message in WHISPER received: %s\n", temp->message);
                                  for (i = 0; i < Node__ClientsDB__size(); i++) {
                                      ConnectedClients t = Node__ClientsDB__get(i);
...
#line 359
                                      if (__nesc_ntoh_uint16(t.srcNode.nxdata) != __nesc_ntoh_uint16(myMsg->src.nxdata) && strcmp(t.username, receiver) == 0) {
                                          ...
                                          data.info = temp->info;
                                          data_address = &data;
                                          sim_log_debug(136U, TRANSPORT_CHANNEL, "%s is UNICASTING to client %s the message: %s\n", temp->username, t.username, temp->message);
                                          Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, __nesc_ntoh_uint16(t.srcNode.nxdata), MAX_TTL, PROTOCOL_ACK, Node__seqNum[sim_node()], (uint8_t *)data_address, PACKET_MAX_PAYLOAD_SIZE);
                                          next = Node__get_next_hop(__nesc_ntoh_uint16(t.srcNode.nxdata));
                                          sim_log_debug(137U, TRANSPORT_CHANNEL, "ACK Msg Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, temp->destPort, __nesc_ntoh_uint16(t.srcNode.nxdata), data.destPort, temp->seqNum);
                                          Node__Sender__send(Node__sendPackage[sim_node()], next);
                                        }
                                    }
                                }
...
#line 395
                                        if (strcmp(t.username, temp->username) == 0) {
                                            ...
                                            data.info = temp->info;
                                            data_address = &data;
                                            sim_log_debug(138U, TRANSPORT_CHANNEL, "SERVER node %d, port %d is sending client list to client %s\n", TOS_NODE_ID, temp->destPort, temp->username);
                                            Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, __nesc_ntoh_uint16(t.srcNode.nxdata), MAX_TTL, PROTOCOL_ACK, Node__seqNum[sim_node()], (uint8_t *)data_address, PACKET_MAX_PAYLOAD_SIZE);
                                            next = Node__get_next_hop(__nesc_ntoh_uint16(t.srcNode.nxdata));
                                            sim_log_debug(139U, TRANSPORT_CHANNEL, "ACK Msg Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, temp->destPort, __nesc_ntoh_uint16(t.srcNode.nxdata), data.destPort, temp->seqNum);
                                            Node__Sender__send(Node__sendPackage[sim_node()], next);
                                          }
                                      }
                                  }
                                    ...
                                    Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, __nesc_ntoh_uint16(myMsg->src.nxdata), MAX_TTL, PROTOCOL_ACK, Node__seqNum[sim_node()], (uint8_t *)data_address, PACKET_MAX_PAYLOAD_SIZE);
                                    next = Node__get_next_hop(__nesc_ntoh_uint16(myMsg->src.nxdata));
                                    sim_log_debug(140U, TRANSPORT_CHANNEL, "ACK Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, temp->destPort, __nesc_ntoh_uint16(myMsg->src.nxdata), temp->srcPort, temp->seqNum);
                                    Node__TCP_Timer__startOneShot(Node__sockets[sim_node()][temp->destPort].RTT * 2);
                                    Node__Sender__send(Node__sendPackage[sim_node()], next);
                                  }
                                }
...
#line 436
                            Node__TCP_Timer__stop();
                            sim_log_debug(141U, TRANSPORT_CHANNEL, "ACK received from node %d port %d to node %d port %d for seqNum %d \n", __nesc_ntoh_uint16(myMsg->src.nxdata), temp->srcPort, TOS_NODE_ID, temp->destPort, temp->seqNum);

                            if (temp->info == 1) {
                                sim_log_debug(142U, TRANSPORT_CHANNEL, "%s BROADCASTED the message: %s\n", temp->username, temp->message);
                              }
                            else {
#line 442
                              if (temp->info == 2) {
                                  sim_log_debug(143U, TRANSPORT_CHANNEL, "%s UNICASTED the message: %s\n", temp->username, temp->message);
                                }
                              else {
#line 445
                                if (temp->info == 3) {
                                    sim_log_debug(144U, TRANSPORT_CHANNEL, "%s sent client list: %s\n", temp->username, temp->message);
                                  }
                                }
                              }
#line 448
                            ...
                            if (temp->seqNum - Node__sockets[sim_node()][temp->destPort].lastAck == 1) {
                                ...
                                sim_log_debug(145U, TRANSPORT_CHANNEL, "Received upto %d packet(s) \n", Node__sockets[sim_node()][temp->destPort].lastAck);
                              }
                          }
                        else {
...
#line 504
                              ...
                              if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_PING) {
                                  ...
                                  sim_log_debug(146U, ROUTING_CHANNEL, "Packet has arrived! %s\n", myMsg->payload);
                                }
                              }
                            }
                          }
                    ...
                    for (i = 0; i < Node__RouteTable__size(); i++) {
                        ...
                        if ((uint16_t )__nesc_ntoh_uint16(myMsg->dest.nxdata) == (uint16_t )__nesc_ntoh_uint16(r.dest.nxdata)) {
                            ...
                            if (__nesc_ntoh_uint16(r.nextHop.nxdata) < 25) {
                                sim_log_debug(147U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, __nesc_ntoh_uint16(r.nextHop.nxdata));
                        ...
                        if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_SYN_ACK) {
                          ...
                          if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_TCP) {
                            ...
                            if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_ACK) {
                                ...
                                sim_log_debug(154U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                                Node__Sender__send(Node__sendPackage[sim_node()], Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                              }
                            }
                          }
...
#line 606
  ...
  sim_log_debug(155U, GENERAL_CHANNEL, "Unknown Packet Type %d\n", len);
  return msg;
}
...
# 78 "/opt/tinyos-main/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC__Receive__receive(am_id_t arg_0x7fce9870de30, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
  switch (arg_0x7fce9870de30) {
#line 78
    case 6:
#line 78
...
#line 78
    default:
#line 78
      __nesc_result = TossimActiveMessageC__Receive__default__receive(arg_0x7fce9870de30, msg, payload, len);
#line 78
      break;
#line 78
    }
...
# 78 "/opt/tinyos-main/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC__Snoop__receive(am_id_t arg_0x7fce9870ca50, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC__Snoop__default__receive(arg_0x7fce9870ca50, msg, payload, len);
#line 78
...
# 110 "/opt/tinyos-main/tos/interfaces/AMSend.nc"
inline static void TossimActiveMessageC__AMSend__sendDone(am_id_t arg_0x7fce98715c70, message_t * msg, error_t error){
#line 110
  /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__AMSend__sendDone(arg_0x7fce98715c70, msg, error);
#line 110
}
#line 110
...
#line 88
# 80 "/opt/tinyos-main/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__AMSend__send(am_id_t arg_0x7fce97ee4b60, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC__AMSend__send(arg_0x7fce97ee4b60, addr, msg, len);
#line 80
...
#line 91
{
  ...
  sim_log_debug(226U, "AMQueue", "AMQueue: request to send from %hhu (%p): passed checks\n", clientId, msg);
  ...
  if (/*AMQueueP.AMQueueImplP*/AMQueueImplP__0__current[sim_node()] >= 1) {
      ...
      sim_log_debug(227U, "AMQueue", "%s: request to send from %hhu (%p): queue empty\n", __FUNCTION__, clientId, msg);
      ...
      if (err != SUCCESS) {
          sim_log_debug(228U, "AMQueue", "%s: underlying send failed.\n", __FUNCTION__);
      ...
      sim_log_debug(229U, "AMQueue", "AMQueue: request to send from %hhu (%p): queue not empty\n", clientId, msg);
    }
  return SUCCESS;
}
...
#line 133
        {
          ...
          sim_log_debug(196U, GENERAL_CHANNEL, "The radio is busy, or something\n");
          return FAIL;
        }
    }
  else 
#line 138
    {
      sim_log_debug(197U, GENERAL_CHANNEL, "The radio is busy");
      return EBUSY;
    }


  sim_log_debug(198U, GENERAL_CHANNEL, "FAILED!?");
  return FAIL;
}
...
# 100 "/opt/tinyos-main/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__Send__sendDone(uint8_t arg_0x7fce97ee5940, message_t * msg, error_t error){
#line 100
  switch (arg_0x7fce97ee5940) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__Send__default__sendDone(arg_0x7fce97ee5940, msg, error);
#line 100
      break;
#line 100
    }
...
#line 680
  sim_log_debug(162U, TRANSPORT_CHANNEL, "result: %s\n", res1);
...
#line 696
    if ((uint8_t )strcmp(res1, "msg") == 0) {
        ...
        sim_log_debug(166U, TRANSPORT_CHANNEL, "message: %s\n", Node__message[sim_node()]);
        Node__instruction[sim_node()] = 1;
        sim_log_debug(167U, TRANSPORT_CHANNEL, "clientPort: %d\n", Node__clientPort[sim_node()]);
...
#line 707
          sim_log_debug(168U, TRANSPORT_CHANNEL, "receiver: %s\n", receiver);
          strcat(Node__user[sim_node()], " ");
          strcat(Node__user[sim_node()], receiver);
          Node__instruction[sim_node()] = 2;
          Node__message[sim_node()] = strtok((void *)0, "\n");
          sim_log_debug(169U, TRANSPORT_CHANNEL, "message: %s\n", Node__message[sim_node()]);
...
#line 716
        if ((uint8_t )strcmp(res1, "listusr") == 0) {
            ...
            sim_log_debug(170U, TRANSPORT_CHANNEL, "%s is not an option\n", res1);
          }
        }
      }
    }
...
#line 619
  sim_log_debug(156U, GENERAL_CHANNEL, "PING EVENT \n");
  sim_log_debug(157U, GENERAL_CHANNEL, "Packet source %d\n", TOS_NODE_ID);

  for (i = 0; i < Node__RouteTable__size(); i++) {
      r = Node__RouteTable__get(i);
      if (destination == (uint16_t )__nesc_ntoh_uint16(r.dest.nxdata) && __nesc_ntoh_uint16(r.cost.nxdata) < 20) {
          sim_log_debug(158U, ROUTING_CHANNEL, "Found a route!!! %d\n", __nesc_ntoh_uint16(r.nextHop.nxdata));
  ...
  if (i == Node__RouteTable__size()) {
      sim_log_debug(159U, GENERAL_CHANNEL, "No Path Found\n");
    }
}
...
#line 22
{
  if (!CommandHandlerP__Queue__empty()) {
      ...
      sim_log_debug(242U, COMMAND_CHANNEL, "A Command has been Issued.\n");
...
# 75 "/opt/tinyos-main/tos/interfaces/TaskBasic.nc"
inline static void SimSchedulerBasicP__TaskBasic__runTask(uint8_t arg_0x7fce988f6020){
#line 75
  switch (arg_0x7fce988f6020) {
#line 75
...
#line 75
    default:
#line 75
      SimSchedulerBasicP__TaskBasic__default__runTask(arg_0x7fce988f6020);
#line 75
      break;
#line 75
    }
...
# 83 "/opt/tinyos-main/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC__0__Timer__fired(uint8_t arg_0x7fce97f96950){
#line 83
  switch (arg_0x7fce97f96950) {
#line 83
    case 0U:
#line 83
...
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC__0__Timer__default__fired(arg_0x7fce97f96950);
#line 83
      break;
#line 83
    }
...
#line 851
{
  ...
  if (nexHop < 25) {
      ...
      sim_log_debug(182U, TRANSPORT_CHANNEL, "Syn Ack Packet sent to Node %d for Port %d with nextHop %d\n", dest_addr, destPort, nexHop);
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
    }
}
...
#line 988
  for (i = 0; i < Node__RouteTable__size(); i++) {
      ...
      if (dest_addr == (uint16_t )__nesc_ntoh_uint16(r.dest.nxdata) && __nesc_ntoh_uint16(r.cost.nxdata) < 255) {
          sim_log_debug(195U, ROUTING_CHANNEL, "next hop: %d\n", __nesc_ntoh_uint16(r.nextHop.nxdata));
...
#line 878
{
  ...
  if (Node__instruction[sim_node()] == 1) {
      sim_log_debug(183U, TRANSPORT_CHANNEL, "in here tcp_send: %s\n", Node__message[sim_node()]);
      ...
      sim_log_debug(184U, TRANSPORT_CHANNEL, "TCP Msg Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, data_address->srcPort, dest_addr, data_address->destPort, Node__nextPacket[sim_node()]);
      Node__sockets[sim_node()][srcPort].effectiveWindow--;
      sim_log_debug(185U, TRANSPORT_CHANNEL, "Updated Effective Window after sending packet to receiver: %d\n", Node__sockets[sim_node()][srcPort].effectiveWindow);
      Node__TCP_Timer__startOneShot(Node__sockets[sim_node()][srcPort].RTT * 2);
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
    }
  if (Node__instruction[sim_node()] == 2) {
      sim_log_debug(186U, TRANSPORT_CHANNEL, "in here tcp_send: %s\n", Node__message[sim_node()]);
      Node__nextPacket[sim_node()]++;
      data.srcPort = srcPort;
      data.destPort = destPort;
      data.seqNum = Node__nextPacket[sim_node()];
      ...
      Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, dest_addr, MAX_TTL, PROTOCOL_TCP, Node__seqNum[sim_node()], (uint8_t *)data_address, PACKET_MAX_PAYLOAD_SIZE);
      sim_log_debug(187U, TRANSPORT_CHANNEL, "TCP WHISPER Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, data_address->srcPort, dest_addr, data_address->destPort, Node__nextPacket[sim_node()]);
      Node__sockets[sim_node()][srcPort].effectiveWindow--;
      sim_log_debug(188U, TRANSPORT_CHANNEL, "Updated Effective Window after sending packet to receiver: %d\n", Node__sockets[sim_node()][srcPort].effectiveWindow);
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
    }
  if (Node__instruction[sim_node()] == 3) {
      sim_log_debug(189U, TRANSPORT_CHANNEL, "in here tcp_send: %s\n", Node__message[sim_node()]);
      Node__nextPacket[sim_node()]++;
      data.srcPort = srcPort;
      data.destPort = destPort;
      data.seqNum = Node__nextPacket[sim_node()];
      ...
      Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, dest_addr, MAX_TTL, PROTOCOL_TCP, Node__seqNum[sim_node()], (uint8_t *)data_address, PACKET_MAX_PAYLOAD_SIZE);
      sim_log_debug(190U, TRANSPORT_CHANNEL, "TCP ListUsr Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, data_address->srcPort, dest_addr, data_address->destPort, Node__nextPacket[sim_node()]);
      Node__sockets[sim_node()][srcPort].effectiveWindow--;
      sim_log_debug(191U, TRANSPORT_CHANNEL, "Updated Effective Window after sending packet to receiver: %d\n", Node__sockets[sim_node()][srcPort].effectiveWindow);
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
    }
  else {
      Node__nextPacket[sim_node()]++;
      ...
      sim_log_debug(192U, TRANSPORT_CHANNEL, "data.username: %s\n", data.username);
      Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, dest_addr, MAX_TTL, PROTOCOL_TCP, Node__seqNum[sim_node()], (uint8_t *)data_address, PACKET_MAX_PAYLOAD_SIZE);
      sim_log_debug(193U, TRANSPORT_CHANNEL, "TCP Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, data_address->srcPort, dest_addr, data_address->destPort, Node__nextPacket[sim_node()]);
      Node__sockets[sim_node()][srcPort].effectiveWindow--;
      sim_log_debug(194U, TRANSPORT_CHANNEL, "Updated Effective Window after sending packet to receiver: %d\n", Node__sockets[sim_node()][srcPort].effectiveWindow);
      Node__TCP_Timer__startOneShot(Node__sockets[sim_node()][srcPort].RTT * 2);
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
    }
}
...
#line 650
{
  ...
  sim_log_debug(160U, TRANSPORT_CHANNEL, "User in setTestClient: %s\n", Node__user[sim_node()]);
  Node__sockets[sim_node()][source_socket].RTT = 15000;
  sim_log_debug(161U, GENERAL_CHANNEL, "RTT: %d\n", Node__sockets[sim_node()][source_socket].RTT);
...
#line 832
  sim_log_debug(179U, GENERAL_CHANNEL, "Target Node: %d\n", dest_addr);
  sim_log_debug(180U, GENERAL_CHANNEL, "Nexthop: %d\n", nexHop);
  data.username = Node__user[sim_node()];
  __nesc_hton_uint16(data.srcPort.nxdata, srcPort);
  __nesc_hton_uint16(data.destPort.nxdata, destPort);
  __nesc_hton_uint16(data.srcNode.nxdata, TOS_NODE_ID);
  ...
  sim_log_debug(181U, GENERAL_CHANNEL, "want to open port: %d\n", __nesc_ntoh_uint16(data.destPort.nxdata));
...
#line 782
  if (Node__RouteTable__size() == 0) {
      ...
      sim_log_debug(174U, ROUTING_CHANNEL, "Creating table for: %d\n", TOS_NODE_ID);
      for (i = 0; i < Node__NeighborList__size(); i++) {
