...
#line 742
typedef int (*__compar_fn_t)(const void *arg_0x14a2bf87a020, const void *arg_0x14a2bf87a320);
#line 780
__extension__ 
#line 797
__extension__ 
...
#line 250
extern double erfc(double arg_0x14a2bf7f25e0) __attribute((__leaf__)) __attribute((__nothrow__)) ;
#line 326
__extension__ 
...
unsigned int (*hashfunction)(void *arg_0x14a2bf5953a0), 
int (*key_eq_fn)(void *arg_0x14a2bf595b70, void *arg_0x14a2bf595e30));
#line 103
#line 102
int 
...
static inline void RandomInitialise(int arg_0x14a2bf382c00, int arg_0x14a2bf382e80);
static double RandomUniform(void );
...
static void Node__CommandHandler__setTestClient(uint16_t source_socket, uint16_t target_addr, uint16_t target_socket, uint16_t data);
#line 8
...
# 64 "/opt/tinyos-main/tos/lib/timer/Timer.nc"
static void Node__periodicTimer1__startPeriodic(uint32_t dt);
# 24 "dataStructures/interfaces/List.nc"
...
#line 136
static uint32_t Node__TCP_Timer__getNow(void );
#line 64
static void Node__TCP_Timer__startPeriodic(uint32_t dt);
...
static inline void Node__createRoutingTable(void );
static void Node__printRoute(void );
...
static inline void Node__CommandHandler__setTestClient(uint16_t source_socket, uint16_t target_addr, uint16_t target_socket, uint16_t data);
#line 497
...
static void CommandHandlerP__CommandHandler__setTestClient(uint16_t source_socket, uint16_t target_addr, uint16_t target_socket, uint16_t data);
#line 8
...
#line 26
# 64 "/opt/tinyos-main/tos/lib/timer/Timer.nc"
inline static void Node__TCP_Timer__startPeriodic(uint32_t dt){
#line 64
...
# 136 "/opt/tinyos-main/tos/lib/timer/Timer.nc"
inline static uint32_t Node__TCP_Timer__getNow(void ){
#line 136
  unsigned int __nesc_result;
#line 136
...
#line 176
                          if (__nesc_ntoh_uint16(temp->dest.nxdata) == __nesc_ntoh_uint16(r->dest.nxdata)) {
                              ...
                              if (__nesc_ntoh_uint16(r->cost.nxdata) < __nesc_ntoh_uint16(temp->cost.nxdata)) {
                                  sim_log_debug(114U, ROUTING_CHANNEL, "Found a better path. Taking: %d\n", __nesc_ntoh_uint16(myMsg->src.nxdata));
...
#line 219
                    index = __nesc_ntoh_uint8(myMsg->payload[1].nxdata);
                    if (Node__sockets[sim_node()][index].state == LISTEN) {
                        Node__TCP_Timer__stop();
                        sim_log_debug(115U, TRANSPORT_CHANNEL, "Syn Packet Arrived from Node %d for Port %d\n", __nesc_ntoh_uint16(myMsg->src.nxdata), index);
                        Node__sockets[sim_node()][index].state = SYN_RCVD;
                        Node__sockets[sim_node()][index].src = TOS_NODE_ID;
                        __nesc_hton_uint16(Node__sockets[sim_node()][index].dest.addr.nxdata, __nesc_ntoh_uint16(myMsg->src.nxdata));
                        __nesc_hton_uint8(Node__sockets[sim_node()][index].dest.port.nxdata, __nesc_ntoh_uint8(myMsg->payload[0].nxdata));
                        Node__sockets[sim_node()][index].RTT = 25000;
                        sim_log_debug(116U, TRANSPORT_CHANNEL, "Values assigned succesfully\n");
                        Node__TCP_Timer__startOneShot(Node__TCP_Timer__getNow() + Node__sockets[sim_node()][i].RTT * 2);
                      ...
                      {
                        sim_log_debug(117U, TRANSPORT_CHANNEL, "state: %s\n", Node__sockets[sim_node()][index].state);
                        sim_log_debug(118U, TRANSPORT_CHANNEL, "Unable to open port because Server port was not open\n");
                      }
                  }
                else {
...
#line 249
                      ...
                      Node__sockets[sim_node()][index].effectiveWindow = __nesc_ntoh_uint8(myMsg->payload[3].nxdata);
                      sim_log_debug(119U, TRANSPORT_CHANNEL, "Connection ESTABLISHED for node %d in port %d\n", TOS_NODE_ID, index);
                      sim_log_debug(120U, TRANSPORT_CHANNEL, "Effective Window: %d\n", Node__sockets[sim_node()][index].effectiveWindow);
                      Node__TCP_Timer__startPeriodic(10000);
                    }
                  else {
#line 257
...
#line 259
                        ...
                        if (Node__sockets[sim_node()][index].state == ESTABLISHED) {
                            ...
                            sim_log_debug(121U, TRANSPORT_CHANNEL, "Terminating server communication for node %d in port %d and changing state to LISTEN\n", TOS_NODE_ID, index);
                          }
                        else {
                            sim_log_debug(122U, TRANSPORT_CHANNEL, "Cannot change state to LISTEN because server port was not created\n");
                          }
                      }
                    else {
...
#line 271
                          if (Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].state == SYN_RCVD) {
                              sim_log_debug(123U, TRANSPORT_CHANNEL, "Changing receiver state to established\n");
                              Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].state = ESTABLISHED;
                            }
                          Node__port_info[sim_node()][0] = __nesc_ntoh_uint8(myMsg->payload[1].nxdata);
                          Node__port_info[sim_node()][1] = __nesc_ntoh_uint8(myMsg->payload[0].nxdata);
                          Node__port_info[sim_node()][2] = __nesc_ntoh_uint8(myMsg->payload[2].nxdata);
                          Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, __nesc_ntoh_uint16(myMsg->src.nxdata), MAX_TTL, PROTOCOL_ACK, Node__seqNum[sim_node()], (uint8_t *)Node__port_info[sim_node()], PACKET_MAX_PAYLOAD_SIZE);
                          next = Node__get_next_hop(__nesc_ntoh_uint16(myMsg->src.nxdata));
                          sim_log_debug(124U, TRANSPORT_CHANNEL, "ACK Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, __nesc_ntoh_uint8(myMsg->payload[1].nxdata), __nesc_ntoh_uint16(myMsg->src.nxdata), __nesc_ntoh_uint8(myMsg->payload[0].nxdata), __nesc_ntoh_uint8(myMsg->payload[2].nxdata));
...
#line 285
                            sim_log_debug(125U, TRANSPORT_CHANNEL, "ACK received from node %d port %d to node %d port %d for seqNum %d \n", __nesc_ntoh_uint16(myMsg->src.nxdata), __nesc_ntoh_uint8(myMsg->payload[0].nxdata), TOS_NODE_ID, __nesc_ntoh_uint8(myMsg->payload[1].nxdata), __nesc_ntoh_uint8(myMsg->payload[2].nxdata));

                            if (Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].effectiveWindow < 3) {
                                Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].effectiveWindow++;
                                sim_log_debug(126U, TRANSPORT_CHANNEL, "Able to send another %d packet(s) from the effective window\n", Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].effectiveWindow);
                              }
                            if (__nesc_ntoh_uint8(myMsg->payload[2].nxdata) - Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].lastAck == 1) {
                                Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].lastAck = __nesc_ntoh_uint8(myMsg->payload[2].nxdata);
                                Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].nextExpected++;
                                if (Node__sockets[sim_node()][__nesc_ntoh_uint8(myMsg->payload[1].nxdata)].effectiveWindow > 0 && Node__nextPacket[sim_node()] <= 250) {
                                    Node__send_TCP(__nesc_ntoh_uint8(myMsg->payload[1].nxdata), __nesc_ntoh_uint16(myMsg->src.nxdata), __nesc_ntoh_uint8(myMsg->payload[0].nxdata));
                                  }
                              }
                          }
                        else {
...
#line 341
                              ...
                              if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_PING) {
                                  ...
                                  sim_log_debug(127U, ROUTING_CHANNEL, "Packet has arrived! %s\n", myMsg->payload);
                                }
                              }
                            }
                          }
...
#line 355
              {
                ...
                if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_LINKEDLIST) {
                    ...
                    if (i == Node__RouteTable__size()) {
                        sim_log_debug(129U, GENERAL_CHANNEL, "No Path Found\n");
                      }
                  }
                else {
                  if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_PING) {
                      ...
                      Node__seqNum[sim_node()]++;
                      Node__addPacketList(Node__sendPackage[sim_node()]);
                      if (Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)) < 25) {
                          sim_log_debug(130U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
...
#line 390
                    if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_SYN) {
                        ...
                        Node__seqNum[sim_node()]++;
                        Node__addPacketList(Node__sendPackage[sim_node()]);
                        sim_log_debug(131U, TRANSPORT_CHANNEL, "REROUTING for the ping event from\n");
                        Node__printRoute();
                        if (Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)) < 25) {
                            sim_log_debug(132U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                            Node__Sender__send(Node__sendPackage[sim_node()], Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                          }
                      }
                    else {
#line 401
                      if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_FIN) {
                          ...
                          Node__addPacketList(Node__sendPackage[sim_node()]);
                          if (Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)) < 25) {
                              sim_log_debug(133U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                              Node__Sender__send(Node__sendPackage[sim_node()], Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                            }
                        }
                      else {
#line 410
                        if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_SYN_ACK) {
                            ...
                            Node__addPacketList(Node__sendPackage[sim_node()]);
                            if (Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)) < 25) {
                                sim_log_debug(134U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                                Node__Sender__send(Node__sendPackage[sim_node()], Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                              }
                          }
                        else {
#line 419
                          if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_TCP) {
                              ...
                              Node__addPacketList(Node__sendPackage[sim_node()]);
                              if (Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)) < 25) {
                                  sim_log_debug(135U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                                  Node__Sender__send(Node__sendPackage[sim_node()], Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                                }
                            }
                          else {
#line 428
                            if (__nesc_ntoh_uint8(myMsg->protocol.nxdata) == PROTOCOL_ACK) {
                                ...
                                Node__addPacketList(Node__sendPackage[sim_node()]);
                                if (Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)) < 25) {
                                    sim_log_debug(136U, ROUTING_CHANNEL, "REROUTING for the ping event from %d to %d\n", TOS_NODE_ID, Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                                    Node__Sender__send(Node__sendPackage[sim_node()], Node__get_next_hop(__nesc_ntoh_uint16(myMsg->dest.nxdata)));
                                  }
                              }
                            }
...
#line 439
  ...
  sim_log_debug(137U, GENERAL_CHANNEL, "Unknown Packet Type %d\n", len);
  return msg;
}
...
# 78 "/opt/tinyos-main/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC__Receive__receive(am_id_t arg_0x14a2bf071e30, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
  switch (arg_0x14a2bf071e30) {
#line 78
    case 6:
#line 78
...
#line 78
    default:
#line 78
      __nesc_result = TossimActiveMessageC__Receive__default__receive(arg_0x14a2bf071e30, msg, payload, len);
#line 78
      break;
#line 78
    }
...
# 78 "/opt/tinyos-main/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC__Snoop__receive(am_id_t arg_0x14a2bf070a50, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC__Snoop__default__receive(arg_0x14a2bf070a50, msg, payload, len);
#line 78
...
#line 104
# 64 "/opt/tinyos-main/tos/lib/timer/Timer.nc"
inline static void Node__periodicTimer1__startPeriodic(uint32_t dt){
#line 64
...
{
  if (err == SUCCESS) {
      Node__periodicTimer__startPeriodic(2000);
      Node__periodicTimer1__startPeriodic(4000);
...
# 110 "/opt/tinyos-main/tos/interfaces/AMSend.nc"
inline static void TossimActiveMessageC__AMSend__sendDone(am_id_t arg_0x14a2bf08bc70, message_t * msg, error_t error){
#line 110
  /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__AMSend__sendDone(arg_0x14a2bf08bc70, msg, error);
#line 110
}
#line 110
...
#line 88
# 80 "/opt/tinyos-main/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__AMSend__send(am_id_t arg_0x14a2be8dab60, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC__AMSend__send(arg_0x14a2be8dab60, addr, msg, len);
#line 80
...
#line 133
        {
          ...
          sim_log_debug(158U, GENERAL_CHANNEL, "The radio is busy, or something\n");
          return FAIL;
        }
    }
  else 
#line 138
    {
      sim_log_debug(159U, GENERAL_CHANNEL, "The radio is busy");
      return EBUSY;
    }


  sim_log_debug(160U, GENERAL_CHANNEL, "FAILED!?");
  return FAIL;
}
...
# 100 "/opt/tinyos-main/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__Send__sendDone(uint8_t arg_0x14a2be8db940, message_t * msg, error_t error){
#line 100
  switch (arg_0x14a2be8db940) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP__0__Send__default__sendDone(arg_0x14a2be8db940, msg, error);
#line 100
      break;
#line 100
    }
...
#line 484
{
  ...
  Node__sockets[sim_node()][source_socket].RTT = data * 100;
  sim_log_debug(142U, GENERAL_CHANNEL, "RTT: %d\n", Node__sockets[sim_node()][source_socket].RTT);
  Node__TCP_Timer__startOneShot(Node__TCP_Timer__getNow() + Node__sockets[sim_node()][source_socket].RTT * 2);
  Node__send_syn(source_socket, __nesc_ntoh_uint16(Node__sockets[sim_node()][source_socket].dest.addr.nxdata), __nesc_ntoh_uint8(Node__sockets[sim_node()][source_socket].dest.port.nxdata));
}

# 9 "lib/interfaces/CommandHandler.nc"
inline static void CommandHandlerP__CommandHandler__setTestClient(uint16_t source_socket, uint16_t target_addr, uint16_t target_socket, uint16_t data){
#line 9
...
#line 452
  sim_log_debug(138U, GENERAL_CHANNEL, "PING EVENT \n");
  sim_log_debug(139U, GENERAL_CHANNEL, "Packet source %d\n", TOS_NODE_ID);

  for (i = 0; i < Node__RouteTable__size(); i++) {
      r = Node__RouteTable__get(i);
      if (destination == (uint16_t )__nesc_ntoh_uint16(r.dest.nxdata) && __nesc_ntoh_uint16(r.cost.nxdata) < 20) {
          sim_log_debug(140U, ROUTING_CHANNEL, "Found a route!!! %d\n", __nesc_ntoh_uint16(r.nextHop.nxdata));
  ...
  if (i == Node__RouteTable__size()) {
      sim_log_debug(141U, GENERAL_CHANNEL, "No Path Found\n");
    }
}
...
#line 22
{
  if (!CommandHandlerP__Queue__empty()) {
      ...
      sim_log_debug(204U, COMMAND_CHANNEL, "A Command has been Issued.\n");
...
# 75 "/opt/tinyos-main/tos/interfaces/TaskBasic.nc"
inline static void SimSchedulerBasicP__TaskBasic__runTask(uint8_t arg_0x14a2bf256020){
#line 75
  switch (arg_0x14a2bf256020) {
#line 75
...
#line 75
    default:
#line 75
      SimSchedulerBasicP__TaskBasic__default__runTask(arg_0x14a2bf256020);
#line 75
      break;
#line 75
    }
...
#line 567
  if (Node__RouteTable__size() == 0) {
      ...
      sim_log_debug(146U, ROUTING_CHANNEL, "Creating table for: %d\n", TOS_NODE_ID);
      for (i = 0; i < Node__NeighborList__size(); i++) {
...
#line 85
  sim_log_debug(110U, TRANSPORT_CHANNEL, "TCP_Timer fired function %d\n", TOS_NODE_ID);
  for (i = 0; i < MAX_NUM_OF_SOCKETS; i++) {
      if (Node__sockets[sim_node()][i].state == SYN_SENT) {
          sim_log_debug(111U, TRANSPORT_CHANNEL, "In SYN_SYN\n");
          Node__TCP_Timer__startOneShot(Node__TCP_Timer__getNow() + Node__sockets[sim_node()][i].RTT * 2);
...
#line 92
        if (Node__sockets[sim_node()][i].state == SYN_RCVD) {
            ...
            sim_log_debug(112U, TRANSPORT_CHANNEL, "In SYN_RCVD\n");
            Node__TCP_Timer__startOneShot(Node__TCP_Timer__getNow() + Node__sockets[sim_node()][i].RTT * 2);
...
#line 98
          if (Node__sockets[sim_node()][i].state == ESTABLISHED) {
              ...
              if (Node__sockets[sim_node()][i].flag == TOS_NODE_ID && Node__nextPacket[sim_node()] <= 250) {

                  Node__sockets[sim_node()][i].nextExpected = Node__nextPacket[sim_node()] + Node__sockets[sim_node()][i].effectiveWindow + 1;
                  for (j = 0; j <= Node__sockets[sim_node()][i].effectiveWindow + 1; j++) {
                      if (Node__sockets[sim_node()][i].effectiveWindow > 0) {
                          Node__TCP_Timeout__startOneShot(Node__TCP_Timer__getNow() + Node__sockets[sim_node()][i].RTT * 2);
...
#line 116
  for (i = 0; i < MAX_NUM_OF_SOCKETS; i++) {
      if (Node__sockets[sim_node()][i].state == ESTABLISHED) {
          if (Node__sockets[sim_node()][i].flag == TOS_NODE_ID && Node__nextPacket[sim_node()] <= 250) {
              sim_log_debug(113U, TRANSPORT_CHANNEL, "Timeout called. Received up to %d packets for going from %d in port %d\n", Node__sockets[sim_node()][i].lastAck, TOS_NODE_ID, Node__sockets[sim_node()][i].src);
              Node__nextPacket[sim_node()] = Node__sockets[sim_node()][i].lastAck;

              Node__sockets[sim_node()][i].effectiveWindow = 3;
            }
        }
    }
}
...
# 83 "/opt/tinyos-main/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC__0__Timer__fired(uint8_t arg_0x14a2be98c950){
#line 83
  switch (arg_0x14a2be98c950) {
#line 83
    case 0U:
#line 83
...
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC__0__Timer__default__fired(arg_0x14a2be98c950);
#line 83
      break;
#line 83
    }
...
#line 628
{
  ...
  Node__port_info[sim_node()][0] = srcPort;
  Node__port_info[sim_node()][1] = destPort;
  Node__port_info[sim_node()][3] = 3;

  Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, dest_addr, MAX_TTL, PROTOCOL_SYN_ACK, Node__seqNum[sim_node()], (uint8_t *)Node__port_info[sim_node()], PACKET_MAX_PAYLOAD_SIZE);
  Node__addPacketList(Node__sendPackage[sim_node()]);
  Node__seqNum[sim_node()]++;
  if (nexHop < 25) {
      sim_log_debug(154U, TRANSPORT_CHANNEL, "Syn Ack Packet sent to Node %d for Port %d with nextHop %d\n", dest_addr, destPort, nexHop);
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
    }
}
...
#line 673
  for (i = 0; i < Node__RouteTable__size(); i++) {
      ...
      if (dest_addr == (uint16_t )__nesc_ntoh_uint16(r.dest.nxdata) && __nesc_ntoh_uint16(r.cost.nxdata) < 255) {
          sim_log_debug(157U, ROUTING_CHANNEL, "next hop: %d\n", __nesc_ntoh_uint16(r.nextHop.nxdata));
...
#line 646
{
  ...
  Node__port_info[sim_node()][0] = srcPort;
  Node__port_info[sim_node()][1] = destPort;
  Node__port_info[sim_node()][2] = Node__nextPacket[sim_node()];
  Node__port_info[sim_node()][3] = Node__sockets[sim_node()][srcPort].effectiveWindow;
  Node__socket[sim_node()] = srcPort;

  Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, dest_addr, MAX_TTL, PROTOCOL_TCP, Node__seqNum[sim_node()], (uint8_t *)Node__port_info[sim_node()], PACKET_MAX_PAYLOAD_SIZE);
  sim_log_debug(155U, TRANSPORT_CHANNEL, "TCP Packet sent from Node %d, port %d to Node %d,Port %d with seqNum:%d\n", TOS_NODE_ID, Node__port_info[sim_node()][0], dest_addr, Node__port_info[sim_node()][1], Node__nextPacket[sim_node()]);



  Node__sockets[sim_node()][srcPort].effectiveWindow--;
  sim_log_debug(156U, TRANSPORT_CHANNEL, "Updated Effective Window after sending packet to receiver: %d\n", Node__sockets[sim_node()][srcPort].effectiveWindow);
  Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
}
...
#line 598
  size = Node__RouteTable__size();
  sim_log_debug(147U, TRANSPORT_CHANNEL, "Getting Route for: %d\n", TOS_NODE_ID);
  if (size == 0) {
      sim_log_debug(148U, ROUTING_CHANNEL, "No Routing found\n");
    }
  else {
      sim_log_debug(149U, TRANSPORT_CHANNEL, "dest | nextHop | cost\n");
      for (i = 0; i < Node__RouteTable__size(); i++) {
          temp1 = Node__RouteTable__get(i);
          sim_log_debug(150U, TRANSPORT_CHANNEL, "%d | %d | %d\n", __nesc_ntoh_uint16(temp1.dest.nxdata), __nesc_ntoh_uint16(temp1.nextHop.nxdata), __nesc_ntoh_uint16(temp1.cost.nxdata));
        }
    }
}
...
#line 615
  sim_log_debug(151U, GENERAL_CHANNEL, "Target Node: %d\n", dest_addr);
  sim_log_debug(152U, GENERAL_CHANNEL, "Nexthop: %d\n", nexHop);
  Node__printRoute();
  Node__port_info[sim_node()][0] = srcPort;
  Node__port_info[sim_node()][1] = destPort;
  sim_log_debug(153U, GENERAL_CHANNEL, "want to open port: %d\n", Node__port_info[sim_node()][1]);
  Node__makePack(&Node__sendPackage[sim_node()], TOS_NODE_ID, dest_addr, MAX_TTL, PROTOCOL_SYN, Node__seqNum[sim_node()], (uint8_t *)Node__port_info[sim_node()], PACKET_MAX_PAYLOAD_SIZE);
  Node__addPacketList(Node__sendPackage[sim_node()]);
  Node__seqNum[sim_node()]++;
  if (nexHop < 25) {
      Node__Sender__send(Node__sendPackage[sim_node()], nexHop);
